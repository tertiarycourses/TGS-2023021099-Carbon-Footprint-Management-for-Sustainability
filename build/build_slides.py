#!/usr/bin/env python3
"""Generate the Carbon Footprint Management course deck in the Tertiary house style.
...
def cover():
    ...
    # simple carbon-management badge: a visual identity without external artwork
    oval(s,Inches(10.82),Inches(0.62),Inches(1.25),Inches(1.25),TEAL)
    txt(s,Inches(10.82),Inches(0.70),Inches(1.25),Inches(0.65),[[('CO₂',25,WHITE,True)]],align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
    txt(s,Inches(10.35),Inches(1.96),Inches(2.2),Inches(0.35),[[('MEASURE · REDUCE',10,TEAL,True)]],align=PP_ALIGN.CENTER)
    txt(s,Inches(0.9),Inches(2.3),Inches(12),Inches(0.6),[[("COURSE SLIDES  ·  WSQ",16,BLUE,True)]])
    txt(s,Inches(0.9),Inches(2.85),Inches(12.0),Inches(1.9),[[(C.COURSE_TITLE,40,INK,True)]])
...
def worked_example(title,intro,code,explain,kicker=None,accent=TEAL):
    ...
    for i,ln in enumerate(code):
        ...
        r=p.add_run(); r.text=ln; r.font.size=Pt(16); r.font.name="Consolas"
    ...
    for i,(lbl,body) in enumerate(explain[:4]):
        ...
        txt(s,ex+Inches(0.26),int(y+Inches(0.1)),ew-Inches(0.45),Inches(0.32),[[(lbl,13,col,True)]])
        txt(s,ex+Inches(0.26),int(y+Inches(0.42)),ew-Inches(0.45),Inches(0.52),[[(_ellipsis(body,72),13,INK,False)]])
    footer(s); return s
...
def chart_slide(title,categories,series,kicker=None,accent=BLUE,
    ...
    ch=gf.chart
    ch.has_title=False
    ch.font.size=Pt(15); ch.font.name="Arial"; ch.font.color.rgb=INK
    ...
    try:
        ...
        dl=pl.data_labels; dl.font.size=Pt(14); dl.font.color.rgb=INK
...
def activity_overview(tag,title,desc,build,services,kicker,objective=None,test=None):
    ...
    txt(s,Inches(0.85),Inches(2.5),Inches(11.63),Inches(1.15),[[(_ellipsis(desc,180),18,INK,False)]])
    ...
    for (col,lbl,body),x in zip(tiles,xs):
        ...
        txt(s,x+Inches(0.24),Inches(4.34),tw-Inches(0.45),Inches(1.4),[[(_ellipsis(body,150),13,INK,False)]])
    footer(s); return s
...
def _render_spec(spec):
    """Dispatch a content specification to a visual slide component."""
    typ=spec["type"]; title=spec["title"]; kicker=spec.get("kicker")
    if typ=="grid":
        return tile_grid(title,spec["items"],kicker=kicker,cols=spec.get("cols",2),size=spec.get("size",14))
    if typ=="process":
        return process_map(title,spec["steps"],kicker=kicker,color=BLUE,
                           synthesis=spec.get("synthesis"))
    if typ=="compare":
        return two_col(title,spec["left"],spec["right"],kicker=kicker,
                       lhead=spec["left_title"],rhead=spec["right_title"])
    if typ=="decision":
        return decision_map(title,spec["question"],
                            ("YES / FIT",spec["yes"]),("NO / NOT YET",spec["no"]),
                            kicker=kicker,note=spec.get("note"))
    if typ=="chart":
        return chart_slide(title,spec["categories"],[('Value',spec["values"])],
                           kicker=kicker,kind="column",insight=spec.get("note"))
    if typ in ("formula","worked"):
        return worked_example(title,spec["intro"],spec["formula"].split("\n"),
                              [(f"CHECK {i+1}",n) for i,n in enumerate(spec["notes"][:4])],
                              kicker=kicker,accent=TEAL)
    if typ=="statement":
        return big_statement(title,spec.get("sub",""),kicker or "KEY IDEA",color=TEAL)
    raise ValueError(f"Unknown slide type: {typ}")

def calculator_visual():
    """Use the authentic SG Carbon Calculator interface as the visual anchor."""
    s=head(slide(),"SG Carbon Calculator: Use It for Screening, Not Inventory", "ACTIVITY 2 · AUTHENTIC TOOL",TEAL)
    path=_logo("sg-carbon-calculator.png")
    if path:
        from PIL import Image
        iw,ih=Image.open(path).size
        boxw,boxh=Inches(5.25),Inches(4.65)
        scale=min(boxw/iw,boxh/ih); w=int(iw*scale); h=int(ih*scale)
        s.shapes.add_picture(path,Inches(0.9)+int((boxw-w)/2),Inches(1.95)+int((boxh-h)/2),width=w,height=h)
    rect(s,Inches(6.45),Inches(1.95),Inches(5.95),Inches(4.65),LIGHT)
    txt(s,Inches(6.8),Inches(2.2),Inches(5.2),Inches(0.4),[[('RIGHT TOOL · RIGHT PURPOSE',13,TEAL,True)]])
    bullets(s,Inches(6.8),Inches(2.75),Inches(5.1),Inches(3.4),[
        "Explore how energy, transport, food and waste choices affect a personal screening result.",
        "Use fictional personas; avoid collecting unnecessary personal information.",
        "Translate insights into engagement hypotheses and operational data needs.",
        "Do not aggregate personal results into a corporate GHG inventory."],size=15,gap=12)
    txt(s,Inches(6.8),Inches(6.15),Inches(5.1),Inches(0.3),[[('alfredang.github.io/sgcarboncalculator',10,GREY,False)]])
    footer(s); return s

def activity_card(a):
    return activity_overview(
        f"ACTIVITY {a['id']}",a["title"],a["objective"],a["deliverable"],
        f"Case: {a['case']}\nSource lens: {a['source']}",
        "REAL-USE-CASE ACTIVITY",objective=a["objective"],
        test="Your recommendation is quantified, evidence-linked and explicit about uncertainty."
    )

# ---------------- ADMINISTRATION ----------------
cover()
section("COURSE ADMINISTRATION","Welcome, Outcomes and Assessment","00","TGS-2023021099 · 2 days · 16 training hours")
tile_grid("Digital Attendance (Mandatory)",[
 ("AM, PM and Assessment","Complete every digital attendance event shown by the trainer."),
 ("Scan and submit","Use your phone camera and verify the submission succeeds."),
 ("Minimum attendance","At least 75% attendance is required for assessment and funding eligibility."),
 ("Ask immediately","Tell the trainer if a scan fails; do not leave it unresolved.")],kicker="TRAQOM · SSG",cols=2,size=14)
trainer_slide("YOUR TRAINER · GENERAL","Your Trainer","To be completed by the trainer",
 [("Name",""),("Designation",""),("Qualifications",""),("Industry experience",""),("Contact","")],"?",GREY)
trainer_slide("YOUR TRAINER",C.TRAINER,"Principal Trainer · Tertiary Infotech Academy",
 [("Focus","Applied AI, sustainability data and business transformation"),("Role","Course facilitation, assessment briefing and learner support"),("Approach","Evidence-led, practical and decision-focused"),("Organisation",C.ORG)],"AA",BLUE)
tile_grid("Let’s Know Each Other",[
 ("You","Name, organisation and role"),("Experience","Exposure to ESG, operations, energy, finance or reporting"),
 ("Challenge","One footprint or sustainability decision you want to improve"),("Outcome","What a useful carbon-reduction plan would enable")],kicker="INTRODUCTIONS",cols=2,size=14)
tile_grid("Ground Rules",["Participate actively and ask questions.","Respect different operational perspectives.","Use fictionalised or approved business data.","Return punctually from breaks.","Keep phones silent outside learning tasks.","Challenge assumptions with evidence."],kicker="HOUSEKEEPING",cols=2,size=14)
tile_grid("Access Your Course Materials",[
 ("LMS","Sign in at lms-tms.tertiaryinfotech.com"),("Course record",C.COURSE_TITLE),
 ("Core files","Slides, Learner Guide and Lesson Plan"),("Activities","One folder per activity with scenario, questions and data"),
 ("Calculator","alfredang.github.io/sgcarboncalculator"),("Assessment","Use only approved open-book materials")],kicker="LEARNING RESOURCES",cols=2,size=13)
tile_grid("Skills Framework Alignment",[
 ("TSC title",C.TSC_TITLE),("TSC code",C.TSC_CODE),("A1","Analyse carbon emission hotspots"),
 ("A2","Prioritise reduction initiatives"),("A3","Develop reduction plans"),("Knowledge","K1–K9 assessed through nine SAQs")],kicker="TSC",cols=2,size=14,accent=VIOLET)
two_col("Learning Journey — Two Days",[
 (C.DAY_THEMES[1],0),("Standards, boundaries and scopes",1),("Inventory calculations and data quality",1),
 ("Hotspot analysis and economics",1),("Activities 1–3",1)],
 [(C.DAY_THEMES[2],0),("Scenarios, technologies and portfolio choices",1),("EMS, governance and Singapore context",1),
 ("Action planning and capstone",1),("Activities 4–9",1)],kicker="SCHEDULE",lhead="DAY 1",rhead="DAY 2")
tile_grid("Learning Outcomes",[(f"LO{i+1}",lo.split("—",1)[-1].strip()) for i,lo in enumerate(C.LEARNING_OUTCOMES)],kicker="BY THE END",cols=1,size=15)
tile_grid("Course Map",[(f"Topic {t['code']} · {t['title']}",f"{t['subtitle']} · {t['ks']} · {t['a']}") for t in C.TOPICS],kicker="THREE CONNECTED TOPICS",cols=1,size=15)
process_map("The Carbon Management Decision Cycle",[
 ("Frame","Boundary and purpose"),("Measure","Activity × factor"),("Diagnose","Hotspots and drivers"),
 ("Prioritise","Carbon, cash and risk"),("Deliver","Plan, control and improve")],kicker="COURSE SPINE",color=TEAL,
 synthesis=("CORE IDEA","Every activity strengthens one part of the same auditable decision trail."))
tile_grid("Assessment Briefing",[
 ("Written Assessment","9 short-answer questions · K1–K9 · 1 hour"),("Case Study","3 applied questions · A1–A3 · 1 hour"),
 ("Open book","Use approved course materials; work independently"),("Evidence","Answer the exact question and show calculations"),
 ("Conduct","No discussion, photography or recording"),("Outcome","Competent or Not Yet Competent per instrument")],kicker="2 HOURS TOTAL",cols=2,size=13,accent=AMBER)
process_map("Assessment Flow",[("TRAQOM survey","Complete feedback"),("Attendance","Scan assessment QR"),("WA","9 SAQs · 1 hour"),("Case Study","3 questions · 1 hour"),("Submit","Follow assessor instructions")],kicker="ASSESSMENT DAY",color=AMBER)
tile_grid("Course Completion and Funding Requirements",[("Attendance","Minimum 75% based on SSG digital attendance"),("Assessment","Complete both assessment instruments"),("TRAQOM","Complete mandatory course feedback"),("Records","Follow administrative and identity-verification requirements")],kicker="WSQ",cols=2,size=14,accent=AMBER)
big_statement("Carbon accounting is not the finish line.","It is the evidence system that helps people choose and deliver better reductions.","START WITH PURPOSE",color=TEAL)

# ---------------- THREE TOPICS ----------------
topic_ranges={}; activity_slides={}
for t,specs,activities in zip(C.TOPICS,[SLIDES1,SLIDES2,SLIDES3],[DOMAIN1,DOMAIN2,DOMAIN3]):
    start=len(prs.slides)+1
    section(f"TOPIC {t['code']} · {t['ks']} · {t['a']}",t["title"],t["code"],t["subtitle"])
    for spec in specs:
        _render_spec(spec)
    for a in activities:
        activity_card(a); activity_slides[str(a["id"])]=len(prs.slides)
        if a["id"]==2: calculator_visual()
    topic_ranges[t["code"]]=[start,len(prs.slides)]
    if t["code"]=="1": brk("LUNCH BREAK","60 minutes")
    elif t["code"]=="2": brk("SHORT BREAK","15 minutes",TEAL)

# ---------------- SOURCES AND CLOSE ----------------
section("EVIDENCE BASE","Standards, Regulation and Further Reading","S","Verify current requirements before external reporting")
for i in range(0,len(C.SOURCES),4):
    tile_grid("Source Register",[(name,urlparse(url).netloc.replace("www.","")) for name,url in C.SOURCES[i:i+4]],kicker=f"REFERENCES · {i+1}–{min(i+4,len(C.SOURCES))} · FULL LINKS IN LEARNER GUIDE",cols=1,size=14,accent=VIOLET)
section("WRAP-UP","From Footprint to Managed Reduction","W","Integrate measurement, decisions and delivery")
tile_grid("What You Can Now Do",[("LO1","Analyse emission hotspots with defensible boundaries and data."),("LO2","Prioritise a portfolio using scenarios, feasibility, economics and transparent criteria."),("LO3","Develop an executable reduction plan with targets, governance and measurement."),("Professional judgement","State assumptions, uncertainty and limitations without hiding them.")],kicker="LEARNING OUTCOMES",cols=2,size=14)
process_map("Your Practical Playbook",[("Frame","Purpose and boundary"),("Quantify","Data and factors"),("Focus","Hotspots"),("Fund","Portfolio"),("Follow through","PDCA and evidence")],kicker="TAKE IT BACK TO WORK",color=TEAL)
tile_grid("Recommended Courses",[(c,"") for c in C.RECOMMENDED_COURSES],kicker="CONTINUE LEARNING",cols=1,size=14)
tile_grid("Support",[("Email","enquiry@tertiaryinfotech.com"),("Telephone","+65 6100 0613"),("Website","www.tertiarycourses.com.sg"),("LMS","lms-tms.tertiaryinfotech.com")],kicker="TERTIARY INFOTECH ACADEMY",cols=2,size=14)
tile_grid("Assessment",[("Written Assessment","9 SAQs · 1 hour · open book"),("Case Study","3 applied questions · 1 hour · open book"),("Digital attendance","Complete the assessment attendance event"),("Submission","Follow the assessor’s stated LMS or hard-copy process")],kicker="FINAL CHECK",cols=2,size=14,accent=AMBER)
process_map("Assessment Flow",[("TRAQOM","Complete survey"),("Attendance","Scan QR"),("WA","K1–K9"),("Case Study","A1–A3"),("Submit","Confirm receipt")],kicker="ON ASSESSMENT DAY",color=AMBER)
tile_grid("Digital Attendance (Mandatory)",[("Complete every event","AM, PM and Assessment attendance"),("Verify success","Do not assume a failed scan is recorded"),("Minimum 75%","Attendance eligibility threshold"),("Ask for help","Resolve issues with the trainer immediately")],kicker="TRAQOM · SSG",cols=2,size=14)
big_statement("Thank You","Measure what matters. Reduce what matters most. Prove what changed.","CARBON MANAGEMENT FOR SUSTAINABILITY",color=TEAL)

# Motion is restrained: content fades; dividers push.
for s in prs.slides:
    texts=" ".join(sh.text_frame.text for sh in s.shapes if sh.has_text_frame)
    is_div=any(x in texts for x in ("COURSE ADMINISTRATION","TOPIC 1 ·","TOPIC 2 ·","TOPIC 3 ·","EVIDENCE BASE","WRAP-UP"))
    _transition(s,"push" if is_div else "fade",speed="med" if is_div else "fast")

os.makedirs(os.path.join(REPO,"courseware"),exist_ok=True)
OUT=os.path.join(REPO,"courseware",f"{C.SHORT_TITLE}-{C.VERSION}.pptx")
prs.save(OUT)

slide_titles={}
for i,s in enumerate(prs.slides,1):
    text=[sh.text_frame.text.strip().split("\n")[0] for sh in s.shapes if sh.has_text_frame and sh.text_frame.text.strip()]
    if text: slide_titles[str(i)]=text[0]
with open(os.path.join(REPO,"build","slide_map.json"),"w",encoding="utf-8") as f:
    json.dump({"slide_count":len(prs.slides),"topics":topic_ranges,"activities":activity_slides,"titles":slide_titles},f,indent=2)
